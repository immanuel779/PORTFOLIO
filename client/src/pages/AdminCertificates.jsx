...
export default function AdminCertificates() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`${import.meta.env.VITE_API_URL}/api/certificates`, { ...formData, image: imageUrl });
            ...
            {certificates.map(cert => (
              <div key={cert.id} className="flex justify-between items-center border border-dark-100 dark:border-dark-700 p-4 rounded-lg">
                <div className="flex items-center gap-4">
                  {cert.image && <img src={cert.image} alt={cert.title} className="w-12 h-12 object-cover rounded-lg" />}
                  <div>
